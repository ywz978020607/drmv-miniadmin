...
class MapSum {
    ...
    public int sum(String prefix) {
        ...
        for(int ii=0;ii<prefix.length();ii++){
            if(temp.next[(int)prefix.charAt(ii)-'a']==null){
                return 0;
            }
            temp = temp.next[(int)prefix.charAt(ii)-'a'];
        }
        dfs(temp,cnt);
        return cnt[0];
    }

    public void dfs(Trie root,int[] cnt){
        Trie temp = root;
        if(temp.isEnd){
            cnt[0] += temp.val;
        }
        ...
        for(int ii=0;ii<26;ii++){
            if(temp.next[ii]!=null){
                dfs(temp.next[ii],cnt);
            }
        }
    }
}
...
class Trie{
    ...
    public void insert(String word,int val){
        ...
        for(int ii=0;ii<word.length();ii++){
            if(temp.next[(int)word.charAt(ii)-'a']==null){
                temp.next[(int)word.charAt(ii)-'a'] = new Trie();
                
            }
            temp = temp.next[(int)word.charAt(ii)-'a'];
        }
        temp.isEnd = true;
        temp.val = val;
    }
