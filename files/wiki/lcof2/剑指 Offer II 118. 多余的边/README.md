...
class Solution {
    public int[] findRedundantConnection(int[][] edges) {
        int n = edges.length;
        graph = new int[n + 1]; // 题目给的从1开始，弄大一点，0:0不用
        ...
        int res = -1;
        for(int ii=0;ii<n;ii++){
            int start = edges[ii][0], end = edges[ii][1];
            if(findpath(start)==findpath(end)){
                res = ii;
            }
            graph[findpath(start)] = findpath(end);
        }
        return (res>=0)?edges[res]:new int[]{};
    }

    private int[] graph;
    public int findpath(int x){
        if(graph[x]!=x){
            graph[x] = findpath(graph[x]);//compress
        }
        return graph[x];
    }
}
