...
class Solution {
    ...
    public void bfs(int[][] mat,int row,int col,int[][] res){
        Deque<int[]> mem = new ArrayDeque<>();
        mem.addLast(new int[]{row,col});

        while(!mem.isEmpty()){
            int[] node = mem.pollFirst();
            for(int dd=0;dd<4;dd++){
                int temp_row = node[0] + direct[dd][0];
                int temp_col = node[1] + direct[dd][1];
                if(temp_row<0||temp_col<0||
                temp_row>=mat.length||temp_col>=mat[0].length){
                    continue;
                }
                if(mat[temp_row][temp_col]==1){
                    if(res[temp_row][temp_col]==0
                    || res[temp_row][temp_col]>res[node[0]][node[1]]+1){
                        //有更新必要
                        res[temp_row][temp_col] = res[node[0]][node[1]]+1;
