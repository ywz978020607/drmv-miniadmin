...
class Solution {
    public int minFlipsMonoIncr(String s) {
        int[] dp = new int[2]; //dp[0]->全是0需要的次数

        for(int ii=0;ii<s.length();ii++){
            if(ii==0){
                dp[0] = s.charAt(ii)=='0'?0:1;
                dp[1] = s.charAt(ii)=='1'?0:1;
            }
            else{
                //每步均计算两种可能
                //注意?: 优先级比较低
                dp[1] = (s.charAt(ii)=='1'?0:1) + Math.min(dp[0],dp[1]); //1前可以全0也可以是1
                dp[0] = (s.charAt(ii)=='0'?0:1) + dp[0]; //0需保证前面都是0
                // if(s.charAt(ii)=='1'){
                //     dp[0] += 1;
                // }
            }
