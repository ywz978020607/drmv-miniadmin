...
public class Codec {
    ...
    public String serialize(TreeNode root) {
        //前序遍历比较方便
        StringBuffer encoder = new StringBuffer(0);
        coder(root,encoder);
        // System.out.println(encoder);
        return encoder.toString();
    }
    public void coder(TreeNode root,StringBuffer encoder){
        if(encoder.length()!=0){
            encoder.append(",");
        }
        if(root==null){
        ...
        //myself
        encoder.append(String.valueOf(root.val));
        
        //dfs
        coder(root.left,encoder);
        coder(root.right,encoder);
    }
    ...
    public TreeNode deserialize(String data) {
        ...
        index = 0;
        TreeNode root = decoder(dataList);
        
        return root;
    }

    public TreeNode decoder(String[] dataList){
        if(dataList[index].equals("#")){
            index++;
            return null;
        }
        TreeNode temp = new TreeNode(Integer.parseInt(dataList[index]));
        index++;

        temp.left = decoder(dataList);
        temp.right = decoder(dataList);
        return temp;
    }
}
