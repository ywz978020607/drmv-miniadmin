...
class Solution {
    public Node insert(Node head, int insertVal) {
        ...
        Node temp = head;
        Node next = temp.next;
        //考虑回环全等（包括只有一个节点）、中间值、新最大、新最小
        while(true){
            if(temp.next==head || (temp.val<=insertVal && next.val>=insertVal) || (temp.val>next.val && (temp.val<=insertVal||next.val>=insertVal))){
                break;
            }
            temp = temp.next;
            next = temp.next;
        ...
        //temp->new->next;
        Node insertNode = new Node(insertVal,next);
