...
class Solution {
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for(int idx = 0; idx < strs.length; idx++){
            char[] tempCharList = strs[idx].toCharArray();
            Arrays.sort(tempCharList);
            // String tempStr = tempCharList.toString(); 错误
            String tempStr = new String(tempCharList);
            System.out.println(tempStr);
            if(map1.containsKey(tempStr)){
                List<String> tempList = map1.get(tempStr);
                tempList.add(strs[idx]);
                map1.put(tempStr, tempList);
            }
            else{
                map1.put(tempStr, new ArrayList<>(Arrays.asList(strs[idx])));
            }
        }
