...
class Solution {
    public String minWindow(String s, String t) {
        ...
        for(int ii=0;ii<t.length();ii++){
            map1.put(t.charAt(ii),map1.getOrDefault(t.charAt(ii),0) + 1);
        }
        //count只记录种类数量 不记录某个数的数量
        int count = map1.size();//计数辅助辨别--减到0后count--

        int left=0,right=0;
        while(right<s.length() || (count==0 && right==s.length())){//右到终点但左可以继续缩也要考虑进去
            //开头判断--此时right还没有添加  故[,) 
            if(count==0){
                //更新min
                if(min_res>right-left){
                    min_res = right-left;
                    start_res = left;
                }
                //缩左
                ...
                if(map1.containsKey(temp)){
                    //存在的元素
                    map1.put(temp,map1.getOrDefault(temp,0)+1);//加回去一个
                    if(map1.getOrDefault(temp,0)==1){
                        //只记录0->1  1->0
                        count++;
                    }
                }
                ...
                map1.put(temp,map1.getOrDefault(temp,0)-1);//减回去一个
        ...
        if(min_res==Integer.MAX_VALUE){
            return new String("");
        }
        else{
            return s.substring(start_res,start_res+min_res);//[,)
        }
