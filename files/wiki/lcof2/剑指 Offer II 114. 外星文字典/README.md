...
class Solution {
    public String alienOrder(String[] words) {
        ...
        for(int ii=1;ii<words.length;ii++){
            String str1 = words[ii-1];
            String str2 = words[ii];
            //！！！！ str2是str1的前缀且str1更长--错误情况判断
            if(str1.startsWith(str2) && !str1.equals(str2)){
                return "";//error
            }
            for(int jj=0;jj<str1.length() && jj<str2.length();jj++){
                //寻找第一个不同的位置
                char ch1 = str1.charAt(jj);
                char ch2 = str2.charAt(jj);
                if(ch1!=ch2){
                    if(!graph.get(ch1).contains(ch2)){
                        graph.get(ch1).add(ch2);
                        inCount.put(ch2,inCount.get(ch2)+1); //ch2的入度++
                    }
                    //跳出
                    break;
                }
        ...
        StringBuffer res = new StringBuffer(26);
        Deque<Character> del = new ArrayDeque<>();
