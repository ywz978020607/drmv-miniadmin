...
class Solution {
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        for(int ii=1;ii<=s1.length();ii++){
            dp[ii][0] = s1.charAt(ii-1) == s3.charAt(ii-1) && dp[ii-1][0];
        }
        for(int jj=1;jj<=s2.length();jj++){
            dp[0][jj] = s2.charAt(jj-1) == s3.charAt(jj-1) && dp[0][jj-1];
        }
        //
        //棋盘格动态规划
        for(int ii=1;ii<= s1.length();ii++){
            for(int jj=1;jj<= s2.length();jj++){
                char ch1 = s1.charAt(ii-1);
                char ch2 = s2.charAt(jj-1);
                char ch3 = s3.charAt(ii+jj-1); //[0,ii-1] [0,jj-1] => [0,ii+jj-1] (len=ii+jj)

                if((ch1==ch3 && dp[ii-1][jj]) || (ch2==ch3 && dp[ii][jj-1])){
                    dp[ii][jj] = true;
                }
                else{
                    dp[ii][jj] = false;
