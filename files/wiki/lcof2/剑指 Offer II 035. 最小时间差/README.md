...
class Solution {
    public int findMinDifference(List<String> timePoints) {
        //小技巧 分钟精度最多只有60*24=1440种
        if(timePoints.size()>1440){
            return 0;
        }

        //思路:直接冲O(n^2);先排序-> O(nlogn);考虑情况优先、设置数组插入空位置O(n)
        //额外考虑一次最开始和最后的时间即可
        boolean[] minutesFlags = new boolean[1440];
        for(String str:timePoints){
            int tempIndex = Integer.parseInt(str.substring(0,2))*60+Integer.parseInt(str.substring(3,5));
            //如果有重复直接返回零
            if(minutesFlags[tempIndex]==true){
                return 0;
            }
            
            minutesFlags[tempIndex] = true;
        }
        ...
        int lastIndex = -1;
        int firstIndex = -1;
        for(int ii=0;ii<minutesFlags.length;ii++){
            if(minutesFlags[ii]==true){
                if(lastIndex>=0){
                    minres = Math.min(minres,ii-lastIndex);//相差的时间
                }
                lastIndex = ii;
                if(firstIndex==-1){
                    firstIndex = ii;//为了最后一次的比较
                }
            }
        }
        minres = Math.min(minres,firstIndex+1440-lastIndex);
        return minres;
    }
}
```
