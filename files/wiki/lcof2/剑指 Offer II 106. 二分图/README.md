...
class Solution {
    int[] node;
    public boolean isBipartite(int[][] graph) {
        node = new int[graph.length];
        for(int ii=0;ii<graph.length;ii++){
            node[ii] = ii;
        }
        // 遍历边
        for(int ii=0;ii<graph.length;ii++){
            int[] op_node_list = graph[ii];
            for(int op_node:op_node_list){
                if(findpath(ii) == findpath(op_node)){
                    return false;
                }
                // 对面一伙
                node[findpath(op_node)] = node[findpath(op_node_list[0])];
            }
        }
        return true;
    }
...
class Solution {
    public boolean isBipartite(int[][] graph) {
        //深搜/广搜
        int[] color = new int[graph.length]; //节点数量 0未分类-> 1,2
        ...
        for(int node=0;node<graph.length;node++){
            if(color[node]==0){
                ...
                int status = 1;
                while(!mem.isEmpty()){
                    Deque<Integer> next_mem = new ArrayDeque<>();
                    while(!mem.isEmpty()){
                        int index = mem.pollFirst();
                        if(color[index]==0){
                            color[index] = status;
                            //添加节点另一头
                            for(int ii=0;ii<graph[index].length;ii++){
                                next_mem.addLast(graph[index][ii]);
                            }
                        }
                        else if(color[index]!=status){
                            return false;
                        }
                    }
                    mem = next_mem;
