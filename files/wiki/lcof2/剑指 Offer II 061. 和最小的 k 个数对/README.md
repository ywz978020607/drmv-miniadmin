...
class Solution {
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        //时间复杂度O(k*k*logk)
        //最小堆- 最小的k个
        PriorityQueue<int[]> heap1 = new PriorityQueue<>((a,b)->b[0]+b[1] - a[0] - a[1]);

        //注意这里选k和长度的最小值
        for(int iter1=0;iter1<Math.min(nums1.length,k);iter1++){
            for(int iter2=0;iter2<Math.min(nums2.length,k);iter2++){
                if(heap1.size()<k){
                    heap1.offer(new int[]{nums1[iter1],nums2[iter2]});
                }
                else if(heap1.peek()[0]+heap1.peek()[1]>nums1[iter1]+nums2[iter2]){
                    heap1.poll();
                    heap1.offer(new int[]{nums1[iter1],nums2[iter2]});
                }
            }
        ...
        while(!heap1.isEmpty()){
            int[] temp = heap1.poll();
            // List<Integer> inserVal = new ArrayList<>();
            // inserVal.add(temp[0]);
            // inserVal.add(temp[1]);
            // res.add(inserVal);
