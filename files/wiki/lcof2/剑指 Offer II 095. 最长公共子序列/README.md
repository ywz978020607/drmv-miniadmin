...
class Solution {
    public int longestCommonSubsequence(String text1, String text2) {
        ...
        //只用两层需要保证len1>len2
        if(len1<len2){
            return longestCommonSubsequence(text2,text1);//互换
        }
        //优化 只用到了两层
        int[][] dp = new int[2][len2+1]; //默认0

        for(int ii=1;ii<=text1.length();ii++){
            for(int jj=1;jj<=text2.length();jj++){
                if(text1.charAt(ii-1)==text2.charAt(jj-1)){
                    dp[ii%2][jj] = 1+ dp[(ii-1)%2][jj-1];
                }
                else{
                    dp[ii%2][jj] = Math.max(dp[(ii-1)%2][jj],dp[ii%2][jj-1]);
                    
                }
                // System.out.println(ii+","+jj+","+dp[ii][jj]);
            }
        }
        return dp[len1%2][len2]; //预留两层0
    }
}
```
...
class Solution {
    public int longestCommonSubsequence(String text1, String text2) {
        int len1 = text1.length();
        int len2 = text2.length();
        int[][] dp = new int[len1+1][len2+1]; //默认0
