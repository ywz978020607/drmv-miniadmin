...
class Solution {
    ...
    public int[] relativeSortArray(int[] arr1, int[] arr2) {
        ...
        //     map2.put(arr2[ii],ii);//放入大小顺序
        // }
        //TreeMap 或用数组int[1001]
        TreeMap<Integer,Integer> map1 = new TreeMap<>();//排序
        for(int ii=0;ii<arr1.length;ii++){
            map1.put(arr1[ii],map1.getOrDefault(arr1[ii],0)+1);
        }
        ...
        int index=0;
        for(int ii=0;ii<arr2.length;ii++){
            if(map1.containsKey(arr2[ii])){
                for(int jj=0;jj<map1.get(arr2[ii]);jj++){
                    res[index] = arr2[ii];
                    index++;
                }
                //remove
        ...
        // System.out.println(entry.getValue());
            for(int jj=0;jj<entry.getValue();jj++){
