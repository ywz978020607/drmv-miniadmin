...
class Solution {
    ...
    public List<List<Integer>> combinationSum2(int[] candidates, int target) {
        //元素重复出现  但只能用一次->排序+剪枝(for/选or不选找next)
    ...
    public void dfs(int[] candidates,int target,int index){
        ...
        dfs(candidates,target-candidates[index],index+1);//不论下一个是否为相同的
        path.remove(path.size()-1);

        //不选 -> 去重 找下一个不同的next_index
        int next_index = index+1;
        while(next_index<candidates.length && candidates[next_index]==candidates[index]){
            next_index++;
        }
        dfs(candidates,target,next_index);
