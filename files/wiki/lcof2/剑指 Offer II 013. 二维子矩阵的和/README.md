...
class NumMatrix {
    ...
    public NumMatrix(int[][] matrix) {
        ...
        for(int ii=0;ii<matrix.length;ii++){
            for(int jj=0;jj<matrix[0].length;jj++){
                sum[ii][jj] = matrix[ii][jj]; //当前元素
                if(ii>0 && jj>0){
                    sum[ii][jj] += sum[ii-1][jj] + sum[ii][jj-1] - sum[ii-1][jj-1];
                }
                else if(ii>0){
                    sum[ii][jj] += sum[ii-1][jj];
                }
                else if(jj>0){
                    sum[ii][jj] += sum[ii][jj-1];
                }
            }
        }
    }
    ...
    public int sumRegion(int row1, int col1, int row2, int col2) {
        ...
        temp+= sum[row2][col2];
        if(row1>0){
            temp -= sum[row1-1][col2];
        }
        if(col1>0){
            temp -= sum[row2][col1-1];
        }
        if(row1>0 && col1>0){
            temp += sum[row1-1][col1-1];
        }
        return temp;
    }
}
