...
class LRUCache {
    ...
    //头尾哑节点
    private Map<Integer,Node> keyToNode; //更改Node内的val之后，map不需要重新调整，因为Map的键值hash没变，存储的对象依旧是Node索引
    private Node addHead,addTail;
    private int capacity;
    ...
    public LRUCache(int capacity) {
        ...
        addHead = new Node();
        addTail = new Node();    
        addHead.next = addTail;
        addTail.prev = addHead;
    }
    ...
    public int get(int key) {
        ...
        Node findNode = keyToNode.get(key);
        //到最前面
        moveToHead(findNode);
        return findNode.val;
    }
    ...
    public void put(int key, int value) {
        if(keyToNode.containsKey(key)){
            //更新
            Node findNode = keyToNode.get(key);
            // keyToNode.remove(key);  // Node作为的是value不是key 不用担心hashcode改变
            findNode.val = value;
            // keyToNode.put(key,findNode);
    ...
    private void addHead(Node temp){
        Node oldFirst = addHead.next;
        oldFirst.prev = temp;
        temp.next = oldFirst;
        ...
        if(keyToNode.size()>this.capacity){
            Node finalNode = addTail.prev;
            keyToNode.remove(finalNode.key); //Node要存key的原因
