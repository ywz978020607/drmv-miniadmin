...
class Solution {
    public boolean findTarget(TreeNode root, int k) {
        //双指针-双迭代器  中序
        Iterator1 iter1 = new Iterator1(root);
        Iterator2 iter2 = new Iterator2(root); //反向迭代器
        if(!iter1.hasNext() || !iter2.hasNext()){
            return false;
        }

        int iter1_val = iter1.next();
        int iter2_val = iter2.next();   

        //利用二叉搜索树特性
        while(iter1_val<iter2_val){
            // System.out.println(iter1_val+","+iter2_val);
            if(iter1_val+iter2_val==k){
                return true;
            }
            else if(iter1_val+iter2_val<k){
                iter1_val = iter1.next();
            }
            else{
                iter2_val = iter2.next();
            }
        }
        return false;
    }
...
class Iterator1{
    ...
    Iterator1(TreeNode root){
        stack1 = new ArrayDeque<>();
        temp = root;
    }

    boolean hasNext(){
        if(temp!=null || !stack1.isEmpty()){
            return true;
        }
        else{
            return false;
