...
class Solution {
    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        findres(candidates,target,0,list1,path);
        return list1;
    }

    public void findres(int[] candidates,int target,int start,List<List<Integer>> list1,List<Integer> path){
        if(target<0){
            return;
        }
        if(target==0){
            //add
            list1.add(new LinkedList<Integer>(path));
            return;
        }
        
        for(int ii=start;ii<candidates.length;ii++){
            //剪枝
           // if(ii>start && candidates[ii]==candidates[ii-1]){
             //   continue;
           // }

            //遍历深搜
            path.add(candidates[ii]);

            findres(candidates,target-candidates[ii],ii,list1,path);
