...
class Solution {
    public String[][] partition(String s) {
        // dfs+isPalinDrome判断  ！！必须用ArrayList 用LinkedList就超时
        // 预处理+动态规划 可以加上  进行优化
        ...
        for(int ii=0;ii<res.size();ii++){
            resString[ii] = new String[res.get(ii).size()];
            for(int jj=0;jj<res.get(ii).size();jj++){
                resString[ii][jj] = res.get(ii).get(jj);
            }
        }
    ...
    //List<List> Deque 混搭
    public void dfs(String s,int start,List<List<String>> res,Deque<String> path){
        if(start==s.length()){
            res.add(new ArrayList<>(path));
            return;
        }
        for(int ii=start;ii<s.length();ii++){
            if(isPalindrome(s,start,ii)){
                path.addLast(s.substring(start,ii+1));
