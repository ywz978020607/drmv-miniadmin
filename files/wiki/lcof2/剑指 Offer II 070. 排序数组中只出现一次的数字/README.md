...
class Solution {
    public int singleNonDuplicate(int[] nums) {
        //注意是排序与否  未排序--异或   排序--二分查找
        int left=0,right=nums.length/2;// 2*iter,2*iter+1
        while(left<=right){
            int mid = (left+right)/2;
            //判断
            if(mid*2<nums.length-1 && nums[mid*2]==nums[mid*2+1]){
                left = mid+1;
            }
            else{
                //判断是否是第一组不同的
                if(mid==0 || nums[mid*2-2]==nums[mid*2-1]){
                    return nums[mid*2]; //返回本组第一个
                }
                //否则
                right = mid-1;
            }
