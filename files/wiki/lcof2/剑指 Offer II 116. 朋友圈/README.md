...
class Solution {
    private int[] graph;
    public int findCircleNum(int[][] isConnected) {
        graph = new int[isConnected.length];
        for(int ii=0;ii<isConnected.length;ii++){
            graph[ii] = ii;
        }
        for(int ii=0;ii<isConnected.length;ii++){
            // 无向图，只用上三角矩阵即可
            for(int jj=ii+1;jj<isConnected.length;jj++){
                if(isConnected[ii][jj]==1){
                    graph[findpath(jj)] = findpath(ii);
                }
            }
        }
...
class Solution {
    ...
    public boolean subdfs(int[][] isConnected,int row,int col){
        ...
        if(isConnected[row][col] == 1){
            ...
            for(int ii=0;ii<isConnected.length;ii++){
                subdfs(isConnected,row,ii);
            }
            for(int ii=0;ii<isConnected[0].length;ii++){
                subdfs(isConnected,col,ii);
            }
            return true;
        }
        return false;
