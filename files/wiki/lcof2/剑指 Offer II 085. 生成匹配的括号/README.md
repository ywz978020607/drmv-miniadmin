...
class Solution {
    public List<String> generateParenthesis(int n) {
        ...
        getdfs(list1,path,n,0);

        return list1;
    }

    public void getdfs(List<String> list1,StringBuffer path,int n,int res){
        if(n==0 && res==0){
            list1.add(new String(path));
        }

        if(n>0){
            path.append('(');

            getdfs(list1,path,n-1,res+1);
